...
```
Read workspace/CLAUDE.md and workspace/01-system/MASTER-INDEX.md. They're templates with
...
files with my answers, and create a 07-brands folder with a BRAND.md for each
...
```
Read the newest file in 03-handoffs and 02-tasks/TASKS.md. In five lines or
...
```
Write a handoff for this session to 03-handoffs/ using TEMPLATE-handoff.md.
...
02-tasks/TASKS.md to match reality.
```
...
sections in 07-brands/example-brand/BRAND.md (voice, customer, offer, hard
rules). Then create 07-brands/[name]/BRAND.md from my answers. Flag anything
...
```
Before we build [thing], draft a brief in 04-briefs/ using TEMPLATE-brief.md.
...
```
Read my last 5 handoffs in 03-handoffs. Find the friction: every place where
...
a folder, a doc, a template, or a small script in 09-scripts. Rank by how
...
(or update) 06-tools/[tool].md: what it's for, how to call it, and the
...
```
Read 02-tasks/TASKS.md, the last 5 handoffs, and 05-ideas/IDEAS.md.
...
```
Read guides/INTEGRATIONS.md. Then ask me which tools I actually use — note app
...
1. **Name the file, not the concept.** "Check the brand voice file at 07-brands/x/BRAND.md" beats "remember our brand voice."
